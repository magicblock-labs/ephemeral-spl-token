...
describe("app", () => {
  ...
  it("appends the fee vault and validator when the payer is delegated", async () => {
    const mint = new PublicKey(DEVNET_USDC_MINT);
    ...
    expect(instruction.keys[5]).toEqual(expect.objectContaining({
      pubkey: magicFeeVaultPdaFromValidator(validator),
      isSigner: false,
      isWritable: true,
    }));
    expect([...instruction.data]).toEqual([5, ...validator.toBuffer()]);
  });

  it("rejects undelegation when the payer is delegated to a different validator", async () => {
    const mint = new PublicKey(DEVNET_USDC_MINT);
    const sendRpcEndpoint = "https://devnet-tee.magicblock.app";
    ...
    vi.spyOn(Connection.prototype, "getMultipleAccountsInfo").mockResolvedValue([
      createDelegationAccountInfo(PublicKey.unique()),
      createDelegationAccountInfo(PublicKey.unique()),
    ]);
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(
    ...
    expect(json.error.code).toBe("VALIDATOR_MISMATCH");
  });
