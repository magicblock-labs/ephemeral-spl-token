...
#[tokio::test]
async fn close_ephemeral_ata_permission() {
    ...
    let ix_close_permission = Instruction {
        program_id: PROGRAM,
        accounts: vec![
            AccountMeta::new(payer, true),
            AccountMeta::new(ephemeral_ata, false),
        ...
        data: vec![instruction::CLOSE_EPHEMERAL_ATA_PERMISSION, bump],
    };

    let tx = Transaction::new_signed_with_payer(
        &[ix_init_ata, ix_create_permission, ix_close_permission],
        Some(&payer),
        &[&context.payer],
