...
pub fn process_undelegate_and_close_shuttle_ephemeral_ata(
...

    {
        ...
        if shuttle_wallet.mint() != &mint || shuttle_wallet.owner() != shuttle_info.address() {
            return Err(ProgramError::InvalidAccountData);
        }
    }
...
///
/// This instruction is intended to be called through DLP call_handler_v2.
///
/// Expected leading accounts (in order):
...
///
/// Trailing accounts (in order from the tail):
/// - second to last: escrow authority account
/// - last:          escrow PDA signer account
