...
///
#[inline(always)]
pub fn process_allocate_transfer_queue(accounts: &[AccountView], _instruction_data: &[u8]) -> ProgramResult {
    let [
    ...
    let realloc_size = {
        ...
        let derived_queue = TransferQueue::derive_pda(&header.mint, &header.validator, header.bump)?;
        ...
        let remaining_capacity = queue_info.lamports() - rent.try_minimum_balance(queue_info.data_len())?;
