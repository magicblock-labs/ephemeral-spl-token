use ephemeral_spl_api::{
    ...
    state::{ephemeral_ata::EphemeralAta, load_initialized, load_mut, shuttle_ephemeral_ata::ShuttleMetadata, RawType},
};
use pinocchio::{
    cpi::Signer,
    error::ProgramError,
...
pub(crate) fn initialize_ephemeral_ata_with_sponsor(
    ...
    if let Ok(ephemeral_ata) = load_initialized::<EphemeralAta>(unsafe { ephemeral_ata_info.borrow_unchecked() }) {
    ...
    if ephemeral_ata_info.data_len() == EPHEMERAL_ATA_V0_LEN && ephemeral_ata_info.owned_by(&crate::ID) {
        ...
        let ephemeral_ata = load_mut::<EphemeralAta>(unsafe { ephemeral_ata_info.borrow_unchecked_mut() })?;
    ...
    require!(ephemeral_ata_info.lamports() == 0, ProgramError::InvalidAccountData);
...
pub(crate) fn initialize_shuttle_ephemeral_ata_with_sponsor(
    ...
    require_eq_keys!(&derived_shuttle_pda, shuttle_info.address(), ProgramError::InvalidSeeds);
    ...
    if !shuttle_is_owned_by_program {
        ...
        let shuttle_seed =
            ShuttleMetadata::signer_seeds(owner_info.address(), mint_info.address(), &shuttle_id_seed, &bump);
        ...
        if shuttle_info.data_len() == SHUTTLE_METADATA_V0_LEN {
            ...
            if current_lamports < Rent::get()?.try_minimum_balance(ShuttleMetadata::LEN)? {
                if let Some(sponsor_signer) = sponsor_signer.clone() {
                    Transfer {
                        from: sponsor_info,
                        to: shuttle_info,
                        lamports: Rent::get()?.try_minimum_balance(ShuttleMetadata::LEN)? - current_lamports,
                    }
                    .invoke_signed(&[sponsor_signer])?;
                } else {
                    Transfer {
            ...
            let shuttle = load_mut::<ShuttleMetadata>(unsafe { shuttle_info.borrow_unchecked_mut() })?;
            shuttle.bump = shuttle_bump;
        }
