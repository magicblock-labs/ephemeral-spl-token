...
pub const SPONSORED_SHUTTLE_DELEGATION_SETUP_LAMPORTS: u64 = if cfg!(feature = "no-fees") {
    0
} else {
    500_000
};
...
pub const SPONSORED_SHUTTLE_PRIVATE_TRANSFER_EXTRA_LAMPORTS: u64 = if cfg!(feature = "no-fees") {
    0
} else {
    1_539_280
};
...
pub const SPONSORED_LAMPORTS_TRANSFER_SETUP_LAMPORTS: u64 = if cfg!(feature = "no-fees") {
    0
} else {
    300_000
};
